<?php

namespace Meklis\TelnetOverProxy;

use meklis\network\Telnet as TelnetClient;

class Telnet extends TelnetClient
{
    protected $proxy_addr = "";
    protected $proxy_timeout = 60;
    protected $lazy_login = "";
    protected $lazy_password = "";
    protected $lazy_host_type = "";

    protected $lazy_connect = false;
    function setLazyConnect($enable = false) {
        $this->lazy_connect = $enable;
        return $this;
    }

    public function disconnect()
    {
        if($this->lazy_connect) {
            return $this;
        }
        return parent::disconnect(); // TODO: Change the autogenerated stub
    }

    public function exec($command, $add_newline = true)
    {
        if($this->lazy_connect && !$this->socket) {
            $this->lazy_connect = false;
            $this->connectOverProxy($this->proxy_addr, $this->timeout);
            $this->login($this->lazy_login, $this->lazy_password, $this->lazy_host_type);
        }
        return parent::exec($command, $add_newline); // TODO: Change the autogenerated stub
    }

    public function login($username, $password, $host_type = 'linux')
    {
        $this->lazy_login = $username;
        $this->lazy_password = $password;
        $this->lazy_host_type = $host_type;
        if($this->lazy_connect) {
            return $this;
        }
        return parent::login($username, $password, $host_type); // TODO: Change the autogenerated stub
    }

    function __construct($host = '127.0.0.1', $port = 23, $timeout = 30, $stream_timeout = 10.0)
    {
        $this->host = $host;
        $this->port = $port;
        $this->timeout = $timeout;
        $this->setStreamTimeout($stream_timeout);

        // set some telnet special characters
        $this->NULL = chr(0);
        $this->DC1 = chr(17);
        $this->WILL = chr(251);
        $this->WONT = chr(252);
        $this->DO = chr(253);
        $this->DONT = chr(254);
        $this->IAC = chr(255);

        // open global buffer stream
        $this->global_buffer = new \SplFileObject('php://temp', 'r+b');
    }

    public function connectOverProxy($proxy_addr = "tcp://127.0.0.1:3333", $timeout = 60) {
        $this->proxy_addr = $proxy_addr;
        $this->proxy_timeout = $timeout;
        if($this->lazy_connect) {
            return $this;
        }
        $ip = "127.0.0.1";
        $port = 3333;
        if(!preg_match('/^(tcp|udp):\/\/(.*?):(.*)$/', $proxy_addr, $matches)) {
            throw new \Exception("Incorrect proxy address for connect");
        }

        // attempt connection - suppress warnings
        $this->socket = @fsockopen($ip, $port, $this->errno, $this->errstr, $timeout);

        $this->setLinuxEOL();

        if (!$this->socket) {
            throw new \Exception("Cannot connect to proxy $ip on port $port");
        }

        // check if we need to convert host to IP
        if (!preg_match('/([0-9]{1,3}\\.){3,3}[0-9]{1,3}/', $this->host)) {
            $ip = gethostbyname($this->host);

            if ($this->host == $ip) {
                throw new \Exception("Cannot resolve $this->host");
            } else {
                $this->host = $ip;
            }
        }

        $this->setPrompt(">>>");
        if (!empty($this->prompt)) {
            $this->waitPrompt();
        }
        try {
            $resp = $this->exec("CONN {$this->host} {$this->port}");
        } catch (\Exception $e) {
            throw new \Exception("Error write connection command to proxy", 1, $e);
        }

        if($resp) {
            list($status, $message) = @explode("###", $resp);
            if(!$status) {
                throw new \Exception("Not correct status response from proxy");
            }
            if($status == "CONNECTION_CLOSED") {
                throw new \Exception("Error connecting to remote device: {$message}");
            }
            if($status == "CONNECTION_LIMIT") {
                throw new \Exception("Connection limit - device is busy. Try connect later.");
            }
            if($status == "CONNECTED") {
                return $this;
            }
        } else {
            throw new \Exception("Incorrect response from proxy");
        }
        $this->setPrompt("");
        return $this;
    }
}